...
function klik(btn){
	...
	else if (sessionStorage.bufor.indexOf('.') != -1 && btn.innerHTML === '.' || sessionStorage.bufor.length >= 12 || sessionStorage.operator == '=') return;
...
function backspace() {
	...
	var txtSliced = '';
	if (txt === '0' || sessionStorage.bufor == sessionStorage.wynik) return;
...
function operand(btn){
	...
	if(sessionStorage.bufor == '0') return;
...
function equalTo(){
	...
	sessionStorage.bufor = sessionStorage.wynik;
	sessionStorage.operator = '=';
}